((window, factory) => {
  ...
  class ChromeTabs {
    ...
    addTab(tabProperties, { animate = true, background = false } = {}) {
      ...
      tabEl.setAttribute("data-properties-original", escape(JSON.stringify(tabProperties)));
      this.tabContentEl.appendChild(tabEl)
      this.setTabCloseEventListener(tabEl)
      this.updateTab(tabEl, tabProperties)
      this.emit('tabAdd', { tabEl })
      this.setupMiddleClickClose(tabEl);
    ...
    setupMiddleClickClose(tabEl){
      tabEl.addEventListener("auxclick", e => {
        if(e.which == 2){
          // tabs remove handled outside
    ...
    setTabCloseEventListener(tabEl) {
      tabEl.querySelector('.chrome-tab-close').addEventListener('click', _ => this.emit('tabRemove', { tabEl }))
    }
